...
class StackoverflowBodyProcessor(StackoverflowProcessor):
    def _select_examples(self, data_dir, mode):
        ...
        if mode == "train":
            X = pickle.load(open("X_Body_train.pickle", "rb"))[:10000]
            Y = pickle.load(open("y_train.pickle", "rb"))[:10000]
        elif mode == "test":
  ...
  def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        pred = tf.dtypes.cast(tf.round(predictions), tf.int32)
        accuracy = avg_jacard(label_ids, pred)
