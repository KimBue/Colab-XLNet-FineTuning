...
class StackoverflowBodyProcessor(StackoverflowProcessor):
    def _select_examples(self, data_dir, mode):
        ...
        if mode == "train":
            X = pickle.load(open("X_Body_train10000.pickle", "rb"))
            Y = pickle.load(open("y_train10000.pickle", "rb"))
        elif mode == "test":
            X = pickle.load(open("X_Body_test10000.pickle","rb"))
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "label_ids": tf.FixedLenSequenceFeature([100], tf.int64),
